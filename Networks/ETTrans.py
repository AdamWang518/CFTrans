...
class CustomEfficientNet(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # Dimension reduction for the last three blocks
        self.dim_reduction1 = DimReduction(self.base_model.feature_info.channels()[-3], 3)
        self.dim_reduction2 = DimReduction(self.base_model.feature_info.channels()[-2], 3)
        self.dim_reduction3 = DimReduction(self.base_model.feature_info.channels()[-1], 3)
        # Convolutional layer to merge outputs and original image
        self.merge_conv = nn.Conv2d(12, 3, kernel_size=1, stride=1, padding=0)

    def forward(self, x):
        # Original image size
        origin=x
        orig_size = x.size()[2:]
        # Get features from the last three blocks
        features = self.base_model(x)
        # Upsample and reduce dimension
        upsampled_features = []
        for feature, dim_reduce in zip(features[-3:], [self.dim_reduction1, self.dim_reduction2, self.dim_reduction3]):
